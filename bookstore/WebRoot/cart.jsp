...
<div class="shopCart container clearfix">
    ...
    <div class="cart-content">
        ...
        <div class="cart-book-item-content">
            ...
            <div class="data-info">
                <div class="data-pic">
                	<input type="hidden" value="${shopcart.pic}" class="pic" style="max-width:150px;">
    ...
    <div class="cart-footer clearfix">
        ...
        <div class="aa">
             <input class="checkpoints" type="checkbox" /> <span>是否使用积分:<c:out value="${points}"></c:out>分</span>
        </div>
        <div class="settle">
...
<script type="text/javascript">
  ...
  //结算
	$('#jiesuan').click(function(){
		var pointFlag=$(".checkpoints").prop("checked");
		var points=${points};
		var changeMoney=0;
		if(points>0){
			changeMoney=(points*1.0)/10;
		}else{
			changeMoney=0;
		}
		var amount=$('#amount').html();
		if(pointFlag){
			amount=amount-changeMoney;
		}
		...
		if(amount=="0.0"){
			...
			var orderList=[];
			if(b==true&&pointFlag==true){//订单已支付，状态为20
				$('.checkbox').each(function(){
					if($(this).is(':checked')){
						var i=0;
						...
						order.tradingType=trading_type;
						order.points=${points};
						orderList.push(order);
						
						
					/* if(firstFlag){
							points=${shopcartList[0].points};
							firstFlag=false;
						}else{
							points=0;
						}
			...
			}else if(b==true&&pointFlag==false){
				$('.checkbox').each(function(){
					if($(this).is(':checked')){
						var id=$(this).parent().find('.id').val();
						var bookId=$(this).parent().find('.bookId').val();
			...
			}else if(b==false&&pointFlag==true){//订单未支付，状态为10
			...
			}else if(b==false&&pointFlag==false){
				var i=0;
				var id=$(this).parent().find('.id').val();
				var bookId=$(this).parent().find('.bookId').val();
				var bookName=$(this).parent().find('.bookName').html();
